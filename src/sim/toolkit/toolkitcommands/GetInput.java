 ...
 * @author user
 */
public class GetInput extends BasicCommand{
    
    private static GetInput singletonInstance;
     
    private String textStringTLV;
    private String responseTLV;
    private String defaultTextTLV;
    private GetInput(){}
    ...
    public static GetInput getInstance()
    {
        ...
        {
            singletonInstance = new GetInput();
        }
    ...
    public void parseCommand(String command) {
        ...
        while(tlvStartOffset<tlvendOffset)
        {
            ...
            switch (tagValue) 
            {
                ...
                case RESPONSE_LENGTH_TAG: case (byte)(RESPONSE_LENGTH_TAG|TAG_SET_CR):
                    temp = getDataLength(command,tlvStartOffset);
                    responseTLV = command.substring(tlvStartOffset, tlvStartOffset+temp);
                    commandTLVList.add(responseTLV);
                    break;
                    
                case DEFAULT_TEXT_TAG: case (byte)(DEFAULT_TEXT_TAG| TAG_SET_CR):
                    temp = getDataLength(command,tlvStartOffset);
                    defaultTextTLV = command.substring(tlvStartOffset, tlvStartOffset+temp);
                    commandTLVList.add(defaultTextTLV);
                    break;
    ...
    public String getDefaultTextTLV()
    {
        return defaultTextTLV;
    }
