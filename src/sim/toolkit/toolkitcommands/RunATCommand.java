 ...
 * @author user
 */
public class RunATCommand extends BasicCommand{
  
    private static RunATCommand singletonInstance;
    private String atCommandTLV;
    private RunATCommand(){}
    
    public static RunATCommand getInstance()
    {
        if(singletonInstance == null)
        {
            singletonInstance = new RunATCommand();
        }
       ...
       int tlvStartOffset =4;
        ...
        while(tlvStartOffset<tlvendOffset)
        {
            ...
            switch (tagValue) 
            {
                ...
                case AT_COMMAND_TAG:case (byte)(AT_COMMAND_TAG|TAG_SET_CR):
                    temp = getDataLength(command,tlvStartOffset);
                    atCommandTLV = command.substring(tlvStartOffset, tlvStartOffset+temp);
                    commandTLVList.add(atCommandTLV);
                    break;
